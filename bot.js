...
${prefix}help-bc / لمعرفت اوامر ال [bc]
...
client.on("message", message => { //clear
     ...
     var guild = client.guilds.get('152601934874935296');
         setInterval(function(){
var role = guild.roles.find(role=> role.name === '®');
if(!role) return;
             role.edit({
                 color : "RANDOM"
             });
...
if (message.content.startsWith(adminprefix + 'sett')) {
      if (!devs.includes(message.author.id)) return;
  client.user.setGame(argresult, "https://www.twitch.tv/xkilleryt");//حقوق دايموند كودز
}
});
